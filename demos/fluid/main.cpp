...
public:
	...
	void update() override {
		...
		if (prev_x != 0 && prev_x != 0) {
			...
			fluid_.AddDensity(prev_x * size_, prev_y * size_, 400, 2);
		}
...
private:
	...
	void mouseMove(ci::app::MouseEvent event) override {
		...
		double x{ event.getX() / double(getWindow()->getSize().x) };
		double y{ 1.0 - event.getY() / double(getWindow()->getSize().y) };
		...
		if (prev_x != 0 || prev_y != 0) {
			...
			fluid_.AddVelocity(x * size_, y * size_, amount_x, amount_y);
		}
